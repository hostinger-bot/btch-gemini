...
export async function versionControl(version: string): Promise<void> {
  if (process.env.JEST_WORKER_ID) { //Saat test skip aja
    return;
  }

  try {
    const response = await axios.get<NpmRegistryResponse>('https://registry.npmjs.org/btch-gemini');
    ...
    console.error('Version check failed:', (er as Error).message || er);
  }
}